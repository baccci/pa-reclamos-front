...
import { useReclamosPorArea } from "@/features/dashboard/empleado/hooks/use-reclamos-por-area"
...
export function ReclamosPorArea() {
  const { reclamos, filtros, isLoading, error, refetch } = useReclamosPorArea()

  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">Reclamos de mi área</h2>
          <p className="text-sm text-muted-foreground">
            Filtrá por estado, fechas y proyecto
          </p>
        </div>
      ...
      {!isLoading && !error && reclamos.length === 0 && (
        <div className="text-center py-10">
          <p className="text-muted-foreground">No hay reclamos para mostrar</p>
        </div>
      )}
          ...
          {reclamos.map((r) => (
            ...
            >
              ...
              <div className="mt-3 text-xs text-muted-foreground">
                Proyecto: {r.proyecto?.nombre ?? r.proyectoId} • Cliente:{" "}
                {r.cliente?.nombre ?? "—"} • Área: {r.areaActual?.nombre ?? "—"}
              </div>
            </Link>
          ))}
        </div>
